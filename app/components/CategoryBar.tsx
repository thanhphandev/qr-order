...
export function CategoryBar() {
  ...
  return (
    <div
      className={`z-40 border-y shadow-sm transition-all duration-300 ${isSticky ? 'sticky top-0 bg-white' : 'relative'
        }`}
    >
      <div className="flex gap-4 px-4 py-3 overflow-x-auto scrollbar-hide">
        {favoriteItems.length > 0 && (
          <button
            ...
            className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg transition-all ${selectedCategory === "favorites"
        ...
        {categories.map((category) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg transition-all ${selectedCategory === category.id
              ? 'bg-orange-500 text-white'
              : 'hover:bg-gray-100 text-gray-700'
              }`}
          >
